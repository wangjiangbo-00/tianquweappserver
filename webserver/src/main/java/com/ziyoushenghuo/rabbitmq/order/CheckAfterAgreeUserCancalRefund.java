...
@Component
@RabbitListener(queues = MQConstant.ORDER_CHECK_CANCALREFUND_NAME)
public class CheckAfterAgreeUserCancalRefund {
    ...
    public void process(String orderid) {
        ...
        try {
            logger.warn("deal seller agree user cancal order and do refund with orderid = " + orderid);
            int oid = Integer.valueOf(orderid);

            Order order = orderService.getOrderById(oid);

            if(order!=null && (order.getOrderStatus() == Order.ORDER_STATUS_TOGIFT || order.getOrderStatus() == Order.ORDER_STATUS_TOSHIP))
            {
                    boolean hasrefund = refundService.checkOrderRefundExist(order.getId(), Refund.REFUND_TYPE_GIVEUP_ORDER);
                    if(hasrefund)
                    {
                        return;
                    }
                    ...
                    refund.setReason(Refund.REFUND_REASONS[ Refund.REFUND_TYPE_GIVEUP_ORDER]);
                    refund.setStatus(Refund.REFUND_STATUS_NORMAL);
                    refund.setType(Refund.REFUND_TYPE_GIVEUP_ORDER);
                    ...
                    try
                    {
                        ...
                        customerSettle.onOrderCancalSettle(order,Refund.REFUND_TYPE_GIVEUP_ORDER);
                    ...
                    if(refundResult.getResult() == WxPayUtils.ORDER_RESULT_OK)
                    {
                        ...
                        order.setFinishreason(Order.ORDER_FINISH_REASON_USER_GIVEUP);
                        order.setOrderStatus(Order.ORDER_STATUS_FINISH);

                        order.setRefundFlag(Refund.GetNewRefundFlag(order.getRefundFlag(),Refund.REFUND_TYPE_GIVEUP_ORDER,Refund.REFUND_STATUS_REFUND_OK));
                    ...
                    else
                    {
                        ...
                        logger.warn("deal seller agree user cancal order and  refund fail with with = " + refundResult.getErrdes() );
                        ...
                        order.setRefundFlag(Refund.GetNewRefundFlag(order.getRefundFlag(),Refund.REFUND_TYPE_GIVEUP_ORDER,Refund.REFUND_STATUS_REFUND_FAIL));
