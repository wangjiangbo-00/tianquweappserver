...
@Component
@RabbitListener(queues = MQConstant.SCHEDULE_DISCOUNT_STOP)
public class DiscountActivityStop {
    ...
    public void process(String id) {
        ...
        try {
            logger.warn("deal with discount stop with id = " + id);
            ...
            if (promotionDiscount != null) {
                ...
                if (promotionDiscount.getType() == PromotionDiscount.RPOMOTION_TYPE_SHOP) {
                    promotionDiscount.setStatus(PromotionDiscount.STATUS_END);
                    promotionDiscountRepository.save(promotionDiscount);
                    promotionDiscountGoodsRepository.updateAllStatusByDiscountId(PromotionDiscount.STATUS_END, gid);
                    ...
                    promotionDiscount.setStatus(PromotionDiscount.STATUS_END);
                    promotionDiscountRepository.save(promotionDiscount);
                    promotionDiscountRepository.updateAllRelatedDiscount(PromotionDiscount.STATUS_END, gid);
                    promotionDiscountGoodsRepository.updateAllStatusByPid(PromotionDiscount.STATUS_END, gid);
                    List<Object> items = new ArrayList<>();
