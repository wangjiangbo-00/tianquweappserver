...
@Component
@RabbitListener(queues = MQConstant.SCHEDULE_DISCOUNT_START)
public class DiscountActivityStart {
    ...
    private static Logger logger = LoggerFactory.getLogger(DiscountActivityStart.class);

    @RabbitHandler
    public void process(String id) {

        try {
            logger.warn("deal with discount start with id = " + id);
            ...
            if(promotionDiscount!=null)
            {
                ...
                {
                    promotionDiscount.setStatus(PromotionDiscount.STATUS_START);
                    promotionDiscountRepository.save(promotionDiscount);
                    promotionDiscountGoodsRepository.updateAllStatusByDiscountId(PromotionDiscount.STATUS_START,gid);
                }
                ...
                else if(promotionDiscount.getType() == PromotionDiscount.RPOMOTION_TYPE_LIMIT)
                {
                    promotionDiscount.setStatus(PromotionDiscount.STATUS_START);
                    promotionDiscountRepository.save(promotionDiscount);
                    promotionDiscountRepository.updateAllRelatedDiscount(PromotionDiscount.STATUS_START,gid);
                    promotionDiscountGoodsRepository.updateAllStatusByPid(PromotionDiscount.STATUS_START,gid);
